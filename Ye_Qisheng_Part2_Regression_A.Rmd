...
##Models using the “base feature” set
#All linear additive features
```{r}
lm_mod_base_1 <- lm(y ~ v1 + v2 + v3 + v4 + x1 + x2 + x3 + x4 + v5 + m, data = df)
summary(lm_mod_base_1)
lm_mod_base_1 %>% readr::write_rds("lm_mod_base_1.rds")
```
#Interaction of the categorical input with all continuous inputs
```{r}
lm_mod_base_2 <- lm(y ~ m*(v1 + v2 + v3 + v4 + v5) + x1 + x2 + x3 + x4, data = df)
summary(lm_mod_base_2)
lm_mod_base_2 %>% readr::write_rds("lm_mod_base_2.rds")
```
#All pair-wise interactions of the continuous inputs
```{r}
lm_mod_base_3 <- lm(y ~ m*(v1 + v2 + v3 + v4 + x1 + x2 + x3 + x4 + v5)*(v5 + x1 + x2 + x3 + x4 + v1 + v2 + v3 + v4), data = df)
summary(lm_mod_base_3)
lm_mod_base_3 %>% readr::write_rds("lm_mod_base_3.rds")
```

##Models using the “expanded feature” set
#Linear additive features
```{r}
lm_mod_expanded_1 <- lm(y ~ w + t + x5 + z, data = df)
summary(lm_mod_expanded_1)
lm_mod_expanded_1 %>% readr::write_rds("lm_mod_expanded_1.rds")
```

#Interaction of the categorical input with continuous features
```{r}
lm_mod_expanded_2 <- lm(y ~ (t + z + w + x5)*(t + z + w + x5), data = df)
summary(lm_mod_expanded_2)
lm_mod_expanded_2 %>% readr::write_rds("lm_mod_expanded_2.rds")
```

#Pair-wise interactions between the continuous features
```{r}
lm_mod_expanded_3 <- lm(y ~m*(t + z + w + x5), data = df)
summary(lm_mod_expanded_3)
lm_mod_expanded_3 %>% readr::write_rds("lm_mod_expanded_3.rds")
```

##3 Models linear basis function models
#1
```{r}
lm_mod_yours_1 <- lm(y ~ (x1 + x2 + x3 + x4 + t + z + w)* m + v1 + v2 + v3 + v4 + v5, data = df)
summary(lm_mod_yours_1)
lm_mod_yours_1 %>% readr::write_rds("lm_mod_yours_1.rds")
```

#2
```{r}
lm_mod_yours_2 <- lm(y ~ (x1 + x2 + x3 + x4 + t + z + w + v1 + v2 + v3 + v4 + v5)*m, data = df)
summary(lm_mod_yours_2)
lm_mod_yours_2 %>% readr::write_rds("lm_mod_yours_2.rds")
```

#3
```{r}
lm_mod_yours_3 <- lm(y ~ (t+z+w+x1+x2+x3+x4+v1+v2+v3+v4+v5)*(t+z+w+x1+x2+x3+x4+v1+v2+v3+v4+v5)+m, data = df)
summary(lm_mod_yours_3)
lm_mod_yours_3 %>% readr::write_rds("lm_mod_yours_3.rds")
```

```{r}
broom::glance(lm_mod_base_1)
broom::glance(lm_mod_base_2)
broom::glance(lm_mod_base_3)
broom::glance(lm_mod_expanded_1)
broom::glance(lm_mod_expanded_2)
broom::glance(lm_mod_expanded_3)
broom::glance(lm_mod_yours_1)
broom::glance(lm_mod_yours_2)
broom::glance(lm_mod_yours_3)
```
By using adjusted r squared, the model9 is the best.

```{r}
coefplot::coefplot(lm_mod_yours_3)
coefplot::coefplot(lm_mod_yours_2)
coefplot::coefplot(lm_mod_yours_1)
```

For the lm_mod_yours_1, x1, x2, x3 and w variables are extremely important for the models while the lm_mod_yours_2 and share the same results.