...
```{r}
lm_glmmod_1 <- glm(outcome ~ x1+x2+x3+x4+v1+v2+v3+v4+v5+m, family = "binomial", data= df)
summary(lm_glmmod_1)
lm_glmmod_1 %>% readr::write_rds("lm_glmmod_1.rds")
```

```{r}
lm_glmmod_2 <- glm(outcome ~ m*(x1+x2+x3+x4+v1+v2+v3+v4+v5), family="binomial" ,data= df)
summary(lm_glmmod_2)
lm_glmmod_2 %>% readr::write_rds("lm_glmmod_2.rds")
```

```{r}
lm_glmmod_3 <- glm(outcome ~ (x1+x2+x3+x4+v1+v2+v3+v4+v5+m)*(x1+x2+x3+x4+v1+v2+v3+v4+v5+m), family="binomial", data = df)
summary(lm_glmmod_3)
lm_glmmod_3 %>% readr::write_rds("lm_glmmod_3.rds")
```
```{r}
lm_glmmod_4 <- glm(outcome~ t+z+w+x5+m, family="binomial",data= df)
summary(lm_glmmod_4)
lm_glmmod_4 %>% readr::write_rds("lm_glmmod_4.rds")
```

```{r}
lm_glmmod_5 <- glm(outcome~m*(t+z+w+x5+m), family="binomial", data= df)
summary(lm_glmmod_5)
lm_glmmod_5 %>% readr::write_rds("lm_glmmod_5.rds")
```

```{r}
lm_glmmod_6 <- glm(outcome ~(t+z+w+x5)*(t+z+w+x5), family="binomial", data= df )
summary(lm_glmmod_6)
lm_glmmod_6 %>% readr::write_rds("lm_glmmod_6.rds")
```

```{r}
lm_glmmod_7 <- glm(outcome ~ x1+x2+x3+x4+v1+v2+v3+v4+v5 + t+z+w+m,family="binomial", data = df)
summary(lm_glmmod_7)
lm_glmmod_7 %>% readr::write_rds("lm_glmmod_7.rds")
```

```{r}
lm_glmmod_8 <- glm(outcome ~ (x1+v1+v2+v3+v4+v5+t+z+w)*m, family="binomial", data = df )
summary(lm_glmmod_8)
lm_glmmod_8 %>% readr::write_rds("lm_glmmod_8.rds")
```
```{r}
lm_glmmod_9 <- glm(outcome ~ (x1+x2+x3+x4+t+z+w)*(x1+x2+x3+x4+t+z+w)+m, family="binomial", data = df )
summary(lm_glmmod_9)
lm_glmmod_9 %>% readr::write_rds("lm_glmmod_9.rds")
```
The performance metric I use to make my selection is AIC.
I consider that the lm_glmmod_7 is the best. The lm_glmmod_7, lm_glmmod_6 and lm_glmmod_4 are top3 models.

```{r}
coefplot::coefplot(lm_glmmod_6)
coefplot::coefplot(lm_glmmod_7)
coefplot::coefplot(lm_glmmod_9)
```
The inputz of x5, z and w are important variable.