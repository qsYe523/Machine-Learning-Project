...
###Part iii: Regression - iiiD) Train/tune with resampling
...
```{r}
linear_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                       + v2 + v3 + v4 + v5 + m,
                       data = df,
                       method = 'glm',
                       metric = my_metric,
...
```{r}
linear_2 <- train(outcome~x1 + x2 + x3 + x4 + x5 + w + z + t + m,
                       data = df,
                       method = 'glm',
                       metric = my_metric,
                       preProcess = c("center", "scale"),
...
```{r}
enet_1 <- train(outcome~(x1 + x2 + x3 + x4)*(v1
                        + v2 + v3 + v4 + v5) + m,
                     data = df,
                     method = 'glmnet',
                     metric = my_metric,
...
```{r}
enet_2 <- train(outcome~m*(x5 + x1 + w + z + t),
                     data = df,
                     method = 'glmnet',
                     metric = my_metric,
                     preProcess = c("center", "scale"),
...
```{r}
nn_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                   + v2 + v3 + v4 + v5 + m,
                   data = df,
                   method = 'nnet',
                   metric = my_metric,
...
```{r}
nn_2 <- train(outcome~x5 + x1 + x2 + x3 + x4 + w + z + t + m,
                   data = df,
                   method = 'nnet',
                   metric = my_metric,
                   preProcess = c("center", "scale"),
...
```{r}
rf_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                   + v2 + v3 + v4 + v5 + m,
                   data = df,
                   method = "rf",
                   metric = my_metric,
...
```{r}
rf_2 <- train(outcome~x5 + x1 + x2 +x3+ x4 + w + z + t + m,
                   data = df,
                   method = "rf",
                   metric = my_metric,
                   trControl = my_ctrl,
...
```{r}
gbt_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                    + v2 + v3 + v4 + v5 + m,
                    data = df,
                    method = "gbm",
                    metric = my_metric,
...
```{r}
gbt_2 <- train(outcome~x5 + x1 +x2 +x3+x4 + w + z + t + m,
                    data = df,
                    method = "gbm",
                    metric = my_metric,
                    trControl = my_ctrl,
...
```{r}
svm_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                    + v2 + v3 + v4 + v5 + m,
                    data = df,
                    method = "svmRadial",
                    metric = my_metric,
...
```{r}
svm_2 <- train(outcome~x5 + x1+x2+x3+x4 + w + z + t + m,
                    data = df,
                    method = "svmRadial",
                    metric = my_metric,
                    trControl = my_ctrl,
...
```{r}
mavs_1 <- train(outcome~x1 + x2 + x3 + x4 + v1
                     + v2 + v3 + v4 + v5 + m,
                     data = df,
                     method = "earth",
                     metric = my_metric,
...
```{r}
mavs_2 <- train(outcome~x5 + x1+x2+x3+x4 + w + z + t + m,
                     data = df,
                     method = "earth",
                     metric = my_metric,
                     trControl = my_ctrl)
...
```{r}
my_ctrl_1 <- trainControl(method = 'repeatedcv', 
                          number = 10, repeats = 3, 
...
```{r}
linear_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1 + v2 + v3 + v4 + v5 + m,
                       data = df,
                       method = 'glm',
                       metric = my_metric_1,
                       preProcess = c("center", "scale"),
...
```{r}
linear_2_roc <- train(outcome~x5 + x1+x2+x3+x4 + w + z + t + m,
                       data = df,
                       method = 'glm',
                       metric = my_metric_1,
                       preProcess = c("center", "scale"),
...
```{r}
enet_1_roc <- train(outcome~(x1 + x2 + x3 + x4 + v1
                        + v2 + v3 + v4 + v5) * m,
                     data = df,
                     method = 'glmnet',
                     metric = my_metric_1,
...
```{r}
enet_2_roc <- train(outcome~(x5 + x1+x2 +x3+ x4 + w + z + t) * m,
                     data = df,
                     method = 'glmnet',
                     metric = my_metric_1,
                     preProcess = c("center", "scale"),
...
```{r}
nn_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1
                   + v2 + v3 + v4 + v5 + m,
                   data = df,
                   method = 'nnet',
                   metric = my_metric_1,
...
```{r}
nn_2_roc <- train(outcome~x5 + x1+ x2+ x3+x4 + w + z + t + m,
                   data = df,
                   method = 'nnet',
                   metric = my_metric_1,
                   preProcess = c("center", "scale"),
...
```{r}
rf_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1
                   + v2 + v3 + v4 + v5 + m,
                   data = df,
                   method = "rf",
                   metric = my_metric_1,
...
```{r}
rf_2_roc <- train(outcome~x5 + x1 + w + z + t + m,
                   data = df,
                   method = "rf",
                   metric = my_metric_1,
                   trControl = my_ctrl_1,
...
```{r}
gbt_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1
                    + v2 + v3 + v4 + v5 + m,
                    data = df,
                    method = "gbm",
                    metric = my_metric_1,
                    trControl = my_ctrl_1,
                    verbose=FALSE)
gbt_1_roc %>% readr::write_rds("gbt_1_roc.rds")
```

```{r}
gbt_2_roc <- train(outcome~x5 + x1+ x2+ x3+x4 + w + z + t + m,
                    data = df,
                    method = "gbm",
                    metric = my_metric_1,
                    trControl = my_ctrl_1,
                    verbose=FALSE)
gbt_2_roc %>% readr::write_rds("gbt_2_roc.rds")
```


```{r}
svm_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1
                    + v2 + v3 + v4 + v5 + m,
                    data = df,
                    method = "svmRadial",
                    metric = my_metric_1,
...
```{r}
svm_2_roc <- train(outcome~x5 + x1 +x2 +x3 +x4 + w + z + t + m,
                    data = df,
                    method = "svmRadial",
                    metric = my_metric_1,
                    trControl = my_ctrl_1,
...
```{r}
mavs_1_roc <- train(outcome~x1 + x2 + x3 + x4 + v1
                     + v2 + v3 + v4 + v5 + m,
                     data = df,
                     method = "earth",
                     metric = my_metric_1,
...
```{r}
mavs_2_roc <- train(outcome~x5 + x1 + w + z + t + m,
                     data = df,
                     method = "earth",
                     metric = my_metric_1,
                     trControl = my_ctrl_1)
...
```{r}
roc_results <- resamples(list(linear_1 = linear_1,
                              linear_2 = linear_2,
                              enet_1 = enet_1,
                              enet_2 = enet_2,
                              nn_1 = nn_1,
...
```{r}
dotplot(roc_results, metric = 'Accuracy')
```


```{r}
roc_results <- resamples(list(linear_1_roc = linear_1_roc,
                              linear_2_roc = linear_2_roc,
                              enet_1_roc = enet_1_roc,
                              enet_2_roc = enet_2_roc,
                              nn_1_roc = nn_1_roc,
                              nn_2_roc = nn_2_roc,
                              rf_1_roc = rf_1_roc,
                              rf_2_roc = rf_2_roc,
                              ...
                              svm_1_roc = svm_1_roc,
                              svm_2_roc = svm_2_roc,
                              mavs_1_roc = mavs_1_roc,
                              mavs_2_roc = mavs_2_roc))
```
