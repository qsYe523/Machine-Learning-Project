...
Yes, the model performance improve.
...
```{r}
viz_1 <- dfnew %>% bind_cols(pred_1 = pred_1)
viz_1 %>% glimpse()
```

```{r}
viz_2 <- dfnew %>% bind_cols(pred_2)
viz_2 %>% glimpse()
```
...
viz_1 %>%  
  ...
  geom_line(mapping = aes(y = pred_1, color = x1)
            , size = 0.8) +
...
viz_2 %>%  
  ...
  geom_line(mapping = aes(y = event, color = w)
            , size = 0.8) +
